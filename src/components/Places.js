...
class Places extends React.Component {
	...
	render() {
		return (
			...
			{
				this.state.places.map((e, i) => <Thumbnail place={e} idPlace={i} likeButton={this.likeButton} key={i}/>)
			}
			</div>
			<div className="thumbnails">
			<Favorites favorites={this.state.places.filter(p => p.liked)}/>
			</div>
			</div>)
	}
}
